...
const JWS_TEST_PUBLIC_KEY = `-----BEGIN PUBLIC KEY-----
MFYwEAYHKoZIzj0CAQYFK4EEAAoDQgAE77bZRAszhhlCqJDokcvjlXOPPPRnAOh+
ZkxZDuo75OAmRZFUJtb3jZQrLRhMa/YwOjUHgSa350GIi5L5oJ59+A==
-----END PUBLIC KEY-----`;

jest.mock('@suite-constants/keys', () => ({
    __esModule: true,
    JWS_PUBLIC_KEY: JWS_TEST_PUBLIC_KEY,
}));

jest.mock('@trezor/suite-data/files/message-system/config.json', () =>
    global.JestMocks.getMessageSystemConfig({ sequence: 10 }),
);
...
describe('Message system actions', () => {
    ...
    describe('init - with invalid JWS', () => {
        ...
        it('raises an error if fetched jws is invalid', async () => {
            ...
            expect(store.getActions()[0].type).toBe(messageSystemConstants.FETCH_CONFIG_ERROR);
            expect(store.getActions()[1].type).toBe(messageSystemConstants.USE_BUNDLED_CONFIG);
            expect(store.getActions()[1].payload).not.toBe(undefined);
            expect(console.error).toHaveBeenCalled();
        });
    });
